...
type UsuarioData = { id: string; nombre: string; email: string; codigo: string; };
...
export default function EmpleadoDashboard() {
  ...
  const init = async () => {
    ...
    const { data: u } = await sb.from("usuarios").select("id,rol,nombre,email,codigo").eq("auth_id", authId).single();
    if (!u) { setError("Usuario no registrado"); setLoading(false); return; }
    if (u.rol !== "empleado") { window.location.href = "/login"; return; }
    setUsuario({ id: u.id, nombre: u.nombre, email: u.email, codigo: u.codigo });

    const { data: emp } = await sb.from("empleados").select("*").eq("auth_id", authId).single();

    if (!emp) {
      const { data: vinc } = await sb.from("vinculaciones").select("empresa_id").eq("profesional_id", u.id).eq("activa", true).single();
      if (!vinc) { setError("Tu cuenta no está vinculada a ningún productor. Contactá al administrador."); setLoading(false); return; }
      const { data: nuevaFicha } = await sb.from("empleados").insert({ empresa_id: vinc.empresa_id, nombre: u.nombre, auth_id: authId, activo: true, permisos: [] }).select().single();
  ...
  if (error) return (
    <div style={{minHeight:"100vh",backgroundImage:"url('/FON.png')",backgroundSize:"cover",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"rgba(255,255,255,0.92)",borderRadius:20,padding:"32px 28px",maxWidth:400,width:"100%",textAlign:"center"}}>
        ...
        <button onClick={logout} style={{background:"none",border:"1.5px solid rgba(25,118,210,0.35)",borderRadius:10,padding:"8px 20px",color:"#1565c0",fontWeight:700,cursor:"pointer",fontSize:13}}>Volver al login</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{minHeight:"100vh",fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",backgroundImage:"url('/FON.png')",backgroundSize:"cover",backgroundPosition:"center",backgroundAttachment:"fixed"}}>
      <style>{`
        ...
        .topbar-dash{background-image:url('/FON.png');background-size:cover;background-position:top center;border-bottom:1px solid rgba(255,255,255,0.40);box-shadow:0 2px 20px rgba(20,80,160,0.14);position:relative;}
        ...
        .mod-card{background-image:url('/FON.png');background-size:cover;background-position:center;border:1.5px solid rgba(255,255,255,0.90);border-top:2px solid rgba(255,255,255,1);border-radius:20px;box-shadow:0 8px 28px rgba(20,80,160,0.15),inset 0 2px 0 rgba(255,255,255,0.90);position:relative;overflow:hidden;cursor:pointer;transition:all 0.22s cubic-bezier(0.34,1.56,0.64,1);}
        ...
        .card-g{background-image:url('/FON.png');background-size:cover;background-position:center;border:1.5px solid rgba(255,255,255,0.90);border-top:2px solid rgba(255,255,255,1);border-radius:20px;box-shadow:0 8px 28px rgba(20,80,160,0.15);position:relative;overflow:hidden;}
        .card-g::before{content:"";position:absolute;inset:0;background:rgba(255,255,255,0.66);pointer-events:none;z-index:0;}
        .card-g>*{position:relative;z-index:1;}
        .tab-btn{padding:9px 18px;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;transition:all 0.18s;background:rgba(255,255,255,0.58);border:1.5px solid rgba(255,255,255,0.92);color:#1e3a5f;}
        .tab-btn.on{background-image:url('/AZUL.png');background-size:cover;border:1.5px solid rgba(100,180,255,0.45);color:white;font-weight:800;box-shadow:0 5px 18px rgba(25,118,210,0.38);text-shadow:0 1px 3px rgba(0,40,120,0.30);}
      ...
      {mounted&&([[8,12,4,2.5,0],[22,45,3,3.5,0.5],[65,8,5,4,0.8],[80,30,3,2.8,1.2],[15,70,4,3.2,0.3],[50,55,3,4.5,1.5],[90,65,5,3,0.7],[35,85,3,2.5,2]] as number[][]).map(([x,y,r,d,delay],i)=>(
        <div key={i} style={{position:"fixed",borderRadius:"50%",background:"white",pointerEvents:"none",left:x+"%",top:y+"%",width:r+"px",height:r+"px",opacity:0.35,zIndex:0,animation:`twinkle ${d}s ease-in-out infinite`,animationDelay:delay+"s"}}/>
      ))}
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 20px",gap:12}}>
          ...
          <div style={{flex:1,textAlign:"center"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,flexWrap:"wrap"}}>
              <span style={{fontSize:15,fontWeight:800,color:"#0d2137"}}>{saludo()}, <span style={{color:"#d97706"}}>{usuario.nombre}</span></span>
              {empresa&&(<span style={{fontSize:12,fontWeight:600,color:"#4a6a8a",display:"flex",alignItems:"center",gap:5}}><span style={{color:"#d97706",fontSize:10}}>◆</span> {empresa.nombre}</span>)}
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",gap:5,padding:"4px 12px",borderRadius:20,background:"rgba(217,119,6,0.10)",border:"1px solid rgba(217,119,6,0.25)"}}>
      ...
      <div style={{maxWidth:1080,margin:"0 auto",padding:"24px 16px 60px",position:"relative",zIndex:1}}>
          ...
          <div className="fade-in">
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:14,marginBottom:20}}>
              {MODULOS.map(mod=>(
                <div key={mod.href} className="mod-card" onClick={()=>window.location.href=mod.href}>
                  <div className="mod-img-wrap"><Image src={mod.img} alt={mod.label} fill style={{objectFit:"cover"}}/></div>
            ...
            {empresa&&(
              <div className="card-g" style={{padding:16}}>
                <div style={{fontSize:11,fontWeight:800,color:"#1e3a5f",textTransform:"uppercase",letterSpacing:1.2,marginBottom:10}}>🏢 Campo asignado</div>
                <div style={{fontSize:18,fontWeight:800,color:"#0d2137"}}>{empresa.nombre}</div>
                <div style={{fontSize:12,color:"#6b8aaa",marginTop:4}}>Código empleado: <strong style={{color:"#d97706"}}>#{usuario.codigo}</strong></div>
              </div>
            )}
          </div>
        )}
          ...
          <div className="fade-in" style={{display:"flex",flexDirection:"column",gap:14}}>
            <div className="card-g" style={{padding:20}}>
              <div style={{display:"flex",alignItems:"center",gap:16}}>
                <div style={{width:60,height:60,borderRadius:"50%",background:"rgba(217,119,6,0.12)",border:"2px solid rgba(217,119,6,0.35)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,flexShrink:0}}>👷</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:20,fontWeight:800,color:"#0d2137"}}>{usuario.nombre}</div>
                  <div style={{fontSize:13,fontWeight:600,color:"#d97706",marginTop:2}}>{empleado.categoria?.replace(/_/g," ")||"Empleado"} · Cód. {usuario.codigo}</div>
                  <div style={{fontSize:11,color:"#6b8aaa"}}>{usuario.email}</div>
                </div>
                <div style={{fontSize:10,padding:"3px 10px",borderRadius:20,fontWeight:700,background:"rgba(22,163,74,0.12)",color:"#16a34a"}}>✓ Activo</div>
              </div>
            </div>
            <div className="card-g" style={{padding:18}}>
              <div style={{fontSize:12,fontWeight:800,color:"#0d2137",textTransform:"uppercase",letterSpacing:0.8,marginBottom:14}}>📋 Datos Personales</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                {[["CUIT",empleado.cuit||"—"],["Teléfono",empleado.telefono||"—"],["Localidad",empleado.localidad||"—"],["Provincia",empleado.provincia||"—"],["Estado civil",empleado.estado_civil||"—"],["Fecha nac.",empleado.fecha_nacimiento||"—"],["Contacto emergencia",empleado.contacto_emergencia||"—"],["Tel. emergencia",empleado.telefono_emergencia||"—"]].map(([l,v])=>(
                  <div key={l} style={{padding:"10px 12px",borderRadius:12,background:"rgba(255,255,255,0.65)",border:"1px solid rgba(180,210,240,0.40)"}}>
            ...
            <div className="card-g" style={{padding:18}}>
              <div style={{fontSize:12,fontWeight:800,color:"#1565c0",textTransform:"uppercase",letterSpacing:0.8,marginBottom:14}}>💼 Datos Laborales</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                {[["Tipo contratación",empleado.tipo_contratacion||"—"],["Jornada",empleado.jornada_horas_dia?empleado.jornada_horas_dia+"hs/día":"—"],["Forma de pago",empleado.forma_pago||"—"],["Alta AFIP",empleado.alta_afip?"✅ Sí":"❌ No"],["Seguridad Social",empleado.alta_seguridad_social?"✅ Sí":"❌ No"],["ART",empleado.art||"—"],["Obra Social",empleado.obra_social||"—"],["Empresa",empresa?.nombre||"—"]].map(([l,v])=>(
                  <div key={l} style={{padding:"10px 12px",borderRadius:12,background:"rgba(255,255,255,0.65)",border:"1px solid rgba(180,210,240,0.40)"}}>
                    <div style={{fontSize:9,fontWeight:700,color:"#6b8aaa",textTransform:"uppercase",letterSpacing:0.8,marginBottom:3}}>{l}</div>
                    <div style={{fontSize:13,fontWeight:700,color:"#0d2137"}}>{v}</div>
                  </div>
        ...
        <p style={{textAlign:"center",marginTop:28,fontSize:11,color:"rgba(30,58,90,0.45)",fontWeight:600,letterSpacing:"0.20em"}}>© AgroGestión PRO 2.8</p>
      </div>
