...
function fmtHa(n: number) { return Math.round(n).toLocaleString("es-AR"); }
...
export default function MargenPage() {
  ...
  const [seccion, setSeccion] = useState<string>("resumen"); // resumen | cultivo:soja | hacienda
  const [usdUsado, setUsdUsado] = useState(1200);
  ...
  const init = async () => {
    ...
    if (!cid && camps?.length) { cid = camps.find(c => c.activa)?.id ?? camps[0].id; }
    ...
    if (cot?.[0]) setUsdUsado(cot[0].usd_usado || 1200);

    await fetchData(emp.id, cid);
    setLoading(false);
  };
  ...
  // ── Hacienda MB ──
  const mbHacienda = hacienda.reduce((a, h) => {
    const valor = h.cantidad * (h.peso_promedio || 0) * (h.precio_kg || 0);
    return a + valor;
  }, 0);
    ...
    <div className="relative min-h-screen bg-[#020810] text-[#E5E7EB]">
      ...
      <div className="relative z-10">
        ...
        <div className="relative px-6 py-3 flex items-center gap-3">
          <button onClick={()=> cultivoActivo || seccion==="hacienda" ? setSeccion("resumen") : window.location.href="/productor/dashboard"}
            className="text-[#4B5563] hover:text-[#C9A227] transition-colors font-mono text-sm">
      ...
      <div className="relative z-10 max-w-6xl mx-auto p-5">
        ...
        {seccion === "resumen" && (
          <div>
            ...
            <div className="mb-6">
              ...
              <p className="text-[#C9A227] text-xs tracking-widest font-mono mt-1">RENTABILIDAD POR CULTIVO Y CAMPAÑA</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
              <div className="card-m p-4 text-center">
                ...
                <div className="text-2xl font-bold font-mono" style={{color:mbHaGeneral>=0?"#4ADE80":"#F87171"}}>${fmtHa(mbHaGeneral)}</div>
              ...
              <div className="card-m p-4 text-center">
                ...
                <div className="text-2xl font-bold text-[#60A5FA] font-mono">USD {fmt(totalMBGeneral / usdUsado)}</div>
                <div className="text-xs text-[#4B5563] font-mono mt-1">TC: ${fmt(usdUsado)}</div>
              </div>
              <div className="card-m p-4 text-center">
            ...
            {cultivosUnicos.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-6">
                {cultivosUnicos.map(cultivo => {
                  const { totalHa, totalMB, mbHa, mgC, estimados, reales } = margenPorCultivo(cultivo);
                  const color = CULTIVO_COLORS[cultivo] ?? "#6B7280";
                  const icon = CULTIVO_ICONS[cultivo] ?? "🌾";
                  const tieneMB = mgC.length > 0;
                  return (
                    <div key={cultivo} className="cult-card card-m overflow-hidden" onClick={() => setSeccion("cultivo:" + cultivo)}
                        ...
                        {tieneMB ? (
                          <div className="grid grid-cols-3 gap-2 text-xs font-mono">
                            <div className="text-center bg-[#020810]/40 rounded-lg p-2">
                              ...
                              <div className="font-bold text-[#E5E7EB]">${fmt(margenPorCultivo(cultivo).totalIngreso/1000000)}M</div>
                            </div>
                            <div className="text-center bg-[#020810]/40 rounded-lg p-2">
                              <div className="text-[#4B5563] mb-1">COSTO</div>
                              <div className="font-bold text-[#F87171]">${fmt(margenPorCultivo(cultivo).totalCosto/1000000)}M</div>
                            </div>
                            <div className="text-center bg-[#020810]/40 rounded-lg p-2">
                              <div className="text-[#4B5563] mb-1">MB/HA</div>
                              <div className="font-bold" style={{color:mbHa>=0?"#4ADE80":"#F87171"}}>${fmtHa(mbHa)}</div>
                            </div>
                          </div>
                        ) : (
                          <div className="text-center py-3 text-[#4B5563] font-mono text-xs">Sin margen cargado — tocá para cargar</div>
                        )}
                ...
                {/* Tarjeta Hacienda si tiene */}
                {tieneHacienda && (
                  <div className="cult-card card-m overflow-hidden" onClick={() => setSeccion("hacienda")}
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-3 pt-3 border-t border-[#A78BFA]/10 flex items-center justify-end">
                        <span className="text-xs text-[#A78BFA] font-mono">Ver detalle →</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {margenes.length > 0 && (
              <div className="card-m overflow-hidden">
                <div className="px-5 py-3 border-b border-[#C9A227]/15 flex items-center justify-between">
                  ...
                  <span className="text-xs text-[#4B5563] font-mono">USD TC: ${fmt(usdUsado)}</span>
                </div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {margenes.map(m => {
                        ...
                        return (
                          <tr key={m.id} className="border-b border-[#C9A227]/5 hover:bg-[#C9A227]/5 cursor-pointer"
                            onClick={() => setSeccion("cultivo:"+m.cultivo)}>
                            ...
                            <td className="px-4 py-3 text-sm text-[#C9A227] font-mono">${fmtHa(m.margen_bruto_ha)}</td>
                      ...
                      <tr className="border-t-2 border-[#C9A227]/30 bg-[#C9A227]/5">
                        ...
                        <td className="px-4 py-3 font-bold text-[#C9A227] font-mono">${fmtHa(mbHaGeneral)}</td>
                        <td/>
                      </tr>
                    </tbody>
                  </table>
        ...
        {cultivoActivo && (() => {
          ...
          // Datos para gráfico torta distribución costos
          ...
          // Datos para gráfico barras por lote
          ...
          // Análisis sensibilidad — primer registro con precio
          ...
          const sensibilidad = ref ? [
            { escenario: "Base", rend: rendRef, precio: precioRef, mbHa: mbHaRef },
            { escenario: "-10% Rend.", rend: rendRef*0.9, precio: precioRef, mbHa: mbHaRef - (rendRef*0.1*precioRef) },
            { escenario: "+10% Rend.", rend: rendRef*1.1, precio: precioRef, mbHa: mbHaRef + (rendRef*0.1*precioRef) },
            { escenario: "-10% Precio", rend: rendRef, precio: precioRef*0.9, mbHa: mbHaRef - (rendRef*precioRef*0.1) },
            { escenario: "+10% Precio", rend: rendRef, precio: precioRef*1.1, mbHa: mbHaRef + (rendRef*precioRef*0.1) },
          ] : [];
            ...
            <div>
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <div className="flex gap-3 flex-wrap">
                      {[
                        ...
                        {l:"MB/HA", v:"$"+fmtHa(mbHa), c:mbHa>=0?"#4ADE80":"#F87171"},
                      ].map(s=>(
              ...
              {mgC.length === 0 ? (
                <div className="card-m p-12 text-center">
                  ...
                  <p className="text-[#4B5563] font-mono text-xs mt-2">Entrá a cada lote y cargá el margen desde la sección de Lotes</p>
                </div>
              ) : (
                  ...
                  <div className="card-m overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs font-mono">
                        <thead><tr className="border-b border-[#C9A227]/10">
                          {["LOTE","HA","REND.","PRECIO","INGRESO","SEMILLA","FERT.","AGROQUÍM.","LABORES","ALQUILER","FLETE","COMERC.","OTROS","COSTO TOTAL","MB","MB/HA","ESTADO"].map(h=>(
                        ...
                        <tbody>
                          {mgC.map(m => {
                            ...
                            return (
                              <tr key={m.id} className="border-b border-[#C9A227]/5 hover:bg-[#C9A227]/5">
                                ...
                                <td className="px-3 py-3 font-bold text-[#C9A227]">${fmtHa(m.margen_bruto_ha)}</td>
                          ...
                          <tr className="border-t-2 border-[#C9A227]/30 bg-[#C9A227]/5">
                            ...
                            <td className="px-3 py-3 font-bold text-[#C9A227]">${fmtHa(mbHa)}</td>
                            <td/>
                          </tr>
                        </tbody>
                      </table>
                  ...
                  {sensibilidad.length > 0 && (
                    <div className="card-m overflow-hidden">
                      <div className="px-5 py-3 border-b border-[#C9A227]/15">
                        ...
                        <span className="text-xs text-[#4B5563] font-mono ml-3">¿Cómo impactan los cambios en el resultado?</span>
                      </div>
                      <div className="overflow-x-auto">
                        <table className="w-full text-xs font-mono">
                          <thead><tr className="border-b border-[#60A5FA]/10">
                            {["ESCENARIO","REND. (tn/ha)","PRECIO ($/tn)","MB/HA"].map(h=>(
                          ...
                          <tbody>
                            {sensibilidad.map((s,i)=>(
                              <tr key={i} className={`border-b border-[#60A5FA]/5 ${i===0?"bg-[#60A5FA]/5 font-bold":""}`}>
                                <td className="px-4 py-3 text-[#E5E7EB]">{s.escenario}{i===0&&" ◀"}</td>
                                <td className="px-4 py-3 text-[#C9A227]">{s.rend.toFixed(1)}</td>
                                <td className="px-4 py-3 text-[#E5E7EB]">${fmt(s.precio)}</td>
                                <td className="px-4 py-3 font-bold" style={{color:s.mbHa>=0?"#4ADE80":"#F87171"}}>${fmtHa(s.mbHa)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
        ...
        {seccion === "hacienda" && (
          <div>
                ...
                return (
                  <div key={h.id} className="card-m p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-[#4ADE80] font-mono">${fmt(valorTotal/1000000)}M</div>
                        <div className="text-xs text-[#4B5563] font-mono">valor estimado</div>
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-xs font-mono">
            ...
            {/* Total hacienda */}
            <div className="card-m p-5 flex items-center justify-between" style={{borderColor:"#A78BFA40"}}>
              <div>
                <div className="text-xs text-[#4B5563] font-mono uppercase tracking-wider">VALOR TOTAL HACIENDA</div>
                <div className="text-xs text-[#4B5563] font-mono mt-0.5">Suma de todas las categorías</div>
              </div>
              <div className="text-3xl font-bold text-[#4ADE80] font-mono">${fmt(mbHacienda)}</div>
            </div>
          </div>
        )}
