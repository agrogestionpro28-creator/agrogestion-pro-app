            ...
            {[0,1,2,3].map(row=>(
              <div key={row} style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,marginBottom:16}}>
                {GRUPOS_MB_LIST.filter((_,i)=>Math.floor(i/3)===row).map((grupo,idx)=>{
                  ...
                  // Para INSUMOS: agrupar directamente por articulo (producto) como categoria
                  // Para otros grupos: mantener logica original por subgrupo
                  const subitems: Record<string,{usd:number;fecha:string;desc:string;origen:string;cantidad?:string}[]> = {};
                  if(grupo.id==="insumos"){
                    // Cada articulo es una categoria separada con sus aplicaciones
                    ...
                    // Logica original para todos los demas grupos
                          ...
                          {Object.keys(subitems).length===0?(
                            ...
                            <div>
                              {Object.entries(subitems).map(([sub,regs])=>{
                                ...
                                if(grupo.id==="insumos"){
                                  // Para INSUMOS: cada producto como categoria con detalle directo
                                ...
                                // Para otros grupos: logica original con expansion por articulo
