...
type Lote = {
  id: string; nombre: string; hectareas: number; tipo_alquiler: string; cultivo: string;
};
type Contrato = {
  ...
  observaciones: string; archivo_url: string; activo: boolean;
};
...
export default function DocumentosPage() {
  ...
  const [showFormContrato, setShowFormContrato] = useState<string | null>(null); // lote_id
  ...
  const init = async () => {
    ...
    await fetchAll(emp.id, campanaActiva);
    setLoading(false);
  };
  ...
  const fetchAll = async (eid: string, campana?: string | null) => {
    ...
    const [docs, emps, conts] = await Promise.all([
  ...
  const subirArchivo = async (file: File, eid: string, carpeta: string = "documentos"): Promise<{ url: string; nombre: string; tipo: string } | null> => {
    ...
    const { error } = await sb.storage.from(carpeta).upload(path, file);
    if (error) { setMsg("Error al subir: " + error.message); return null; }
    const { data: urlData } = sb.storage.from(carpeta).getPublicUrl(path);
  ...
  const guardarContrato = async (loteId: string) => {
    ...
    let archivoUrl = "";
    if (contratoArchivo) {
      const result = await subirArchivo(contratoArchivo, empresaId, "documentos");
      if (result) archivoUrl = result.url;
    }
    // Ver si ya existe contrato para ese lote
    const existing = contratos.find(c => c.lote_id === loteId);
    if (existing) {
      await sb.from("contratos_alquiler").update({
      ...
      await sb.from("contratos_alquiler").insert({
        empresa_id: empresaId, lote_id: loteId,
        propietario_nombre: form.propietario_nombre ?? "",
        propietario_telefono: form.propietario_telefono ?? "",
        propietario_email: form.propietario_email ?? "",
        fecha_inicio: form.fecha_inicio || null,
        ...
        unidad: form.unidad ?? "",
        observaciones: form.observaciones ?? "",
        archivo_url: archivoUrl, activo: true,
      });
    }
  ...
  // Calcular días para vencimiento del contrato
  ...
  // Alertas contratos por vencer (60 días)
  ...
  const formatCondicion = (c: Contrato) => {
    switch(c.condicion) {
      ...
      case "quintales": return `${c.monto} qq/ha`;
              ...
              {alertasContratos.map(c => {
                ...
                return (
                  <div key={c.id} className={`px-3 py-1.5 rounded-lg text-xs font-mono border ${dias !== null && dias <= 30 ? "border-[#F87171]/30 text-[#F87171] bg-[#F87171]/5" : "border-[#C9A227]/30 text-[#C9A227] bg-[#C9A227]/5"}`}>
                    🏘️ {dias !== null && dias <= 0 ? "VENCIDO" : `${dias} días`} · Contrato {lote?.nombre ?? c.propietario_nombre}
                  </div>
                );
              })}
              {alertasVenc.map(d => {
          ...
          <div className="w-64 flex-shrink-0">
            ...
            <div className="mt-4 bg-[#0a1628]/80 border border-[#00FF80]/15 rounded-xl p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-xs font-mono">
                  <span className="text-[#4B5563]">Contratos activos</span>
                  <span className="text-[#C9A227] font-bold">{contratos.length}</span>
                </div>
                <div className="flex justify-between text-xs font-mono">
                ...
                <div className="flex justify-between text-xs font-mono">
                  ...
                  <span className="text-[#F87171] font-bold">{alertasVenc.length + alertasContratos.length}</span>
                </div>
              </div>
            </div>
          </div>
                          ...
                          {documentos.slice(0, 8).map(d => {
                            ...
                            return (
                              <tr key={d.id} className="border-b border-[#00FF80]/5 hover:bg-[#00FF80]/5 transition-colors">
                                ...
                                <td className="px-4 py-3 text-xs font-mono" style={{ color: vencido ? "#F87171" : "#9CA3AF" }}>{d.fecha_vencimiento || "—"}</td>
              ...
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#C9A227] font-mono">{lotes.length} lotes no propios · {contratos.length} contratos cargados</p>
                  </div>
                </div>

                {lotes.length === 0 ? (
                  <div className="text-center py-20 bg-[#0a1628]/60 border border-[#C9A227]/15 rounded-xl mt-6">
                    <div className="text-5xl mb-4 opacity-20">🏘️</div>
                    <p className="text-[#4B5563] font-mono">No hay lotes alquilados en la campaña activa</p>
                  ...
                  <div className="space-y-4 mt-4">
                    {lotes.map(lote => {
                      ...
                      return (
                        <div key={lote.id} className={`bg-[#0a1628]/80 border rounded-xl overflow-hidden transition-all ${vencido ? "border-[#F87171]/30" : porVencer30 ? "border-[#F87171]/20" : porVencer60 ? "border-[#C9A227]/30" : "border-[#C9A227]/15"}`}>
                          {/* Header del lote */}
                          <div className="px-5 py-4 flex items-center justify-between flex-wrap gap-3">
                            <div className="flex items-center gap-4">
                              <div className="w-10 h-10 rounded-xl bg-[#C9A227]/10 border border-[#C9A227]/30 flex items-center justify-center font-bold text-[#C9A227] font-mono text-sm">
                                {lote.hectareas}
                              </div>
                              <div>
                                ...
                                onClick={() => {
                                  if (editando) { setShowFormContrato(null); setForm({}); }
                                    ...
                                    if (contrato) {
                                      ...
                                      setForm({ condicion: "fijo_pesos", moneda: "ARS" });
                                    }
                                  }
                                }}
                            ...
                            <div className="px-5 pb-4 grid grid-cols-2 md:grid-cols-4 gap-3">
                              <div className="bg-[#020810]/40 rounded-lg p-3">
                                ...
                                {contrato.propietario_telefono && (
                                  <a href={`https://wa.me/54${contrato.propietario_telefono.replace(/\D/g,"")}?text=Hola ${contrato.propietario_nombre}!`}
                                    ...
                                    className="text-xs text-[#25D366] font-mono mt-1 block hover:underline">
                                    💬 {contrato.propietario_telefono}
                                  </a>
                                )}
                              </div>
                              <div className="bg-[#020810]/40 rounded-lg p-3">
                                ...
                                <div className="text-xs text-[#4B5563] font-mono mt-1">
                                  Total: {contrato.condicion === "fijo_pesos" ? `$${(Number(contrato.monto) * lote.hectareas).toLocaleString("es-AR")}` :
                                    contrato.condicion === "fijo_usd" ? `USD ${Number(contrato.monto) * lote.hectareas}` :
                                    contrato.condicion === "quintales" ? `${Number(contrato.monto) * lote.hectareas} qq` : "—"}
                                </div>
                              </div>
                              <div className="bg-[#020810]/40 rounded-lg p-3">
                              ...
                              <div className="bg-[#020810]/40 rounded-lg p-3">
                                <div className="text-xs text-[#4B5563] font-mono mb-1">ACCIONES</div>
                                {contrato.archivo_url && (
                                  <a href={contrato.archivo_url} target="_blank" rel="noreferrer" className="text-xs text-[#00FF80] font-mono block hover:underline">📎 Ver contrato</a>
                                )}
                            ...
                            <div className="px-5 pb-5 border-t border-[#C9A227]/15 pt-4">
                              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                ...
                                <div><label className={labelClass}>Email</label>
                                  <input type="email" value={form.propietario_email ?? ""} onChange={e => setForm({...form, propietario_email: e.target.value})} className={inputClass} placeholder="email@ejemplo.com" />
                                </div>
                                <div><label className={labelClass}>Condición de alquiler</label>
                                  <select value={form.condicion ?? "fijo_pesos"} onChange={e => setForm({...form, condicion: e.target.value})} className={inputClass}>
                                    {CONDICIONES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                                  </select>
                                </div>
                                <div><label className={labelClass}>
                                  {form.condicion === "fijo_pesos" ? "Monto $/ha" : form.condicion === "fijo_usd" ? "Monto USD/ha" : form.condicion === "quintales" ? "Quintales/ha" : "% producción"}
                                </label>
                                ...
                                {form.condicion === "quintales" && (
                                  <div><label className={labelClass}>Especie (soja, trigo...)</label>
                                    <input type="text" value={form.unidad ?? ""} onChange={e => setForm({...form, unidad: e.target.value})} className={inputClass} placeholder="Ej: soja" />
                                  </div>
                                )}
                                ...
                                <div><label className={labelClass}>Observaciones</label>
                                  <input type="text" value={form.observaciones ?? ""} onChange={e => setForm({...form, observaciones: e.target.value})} className={inputClass} placeholder="Notas del contrato" />
                                </div>
                                <div className="md:col-span-3">
                                  <label className={labelClass}>Adjuntar contrato (PDF)</label>
                                  <div className="flex items-center gap-3">
                                    <input ref={contratoFileRef} type="file" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" className="hidden"
                                      onChange={e => setContratoArchivo(e.target.files?.[0] ?? null)} />
                                    <button onClick={() => contratoFileRef.current?.click()}
                                      className="px-4 py-2 border border-[#C9A227]/30 text-[#C9A227] rounded-xl text-sm font-mono hover:bg-[#C9A227]/10 transition-all">
                                      📎 Seleccionar PDF
                                    </button>
                              ...
                              <div className="flex gap-3 mt-4">
                                ...
                                <button onClick={() => { setShowFormContrato(null); setForm({}); setContratoArchivo(null); }}
                                  className="border border-[#1C2128] text-[#4B5563] px-6 py-2.5 rounded-xl text-sm font-mono">Cancelar</button>
                              </div>
                            </div>
                          )}
                        </div>
              ...
              <div>
                ...
                {showForm && (
                  <div className="bg-[#0a1628]/80 border border-[#FB923C]/30 rounded-xl p-5 mb-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div><label className={labelClass}>Nombre completo</label><input type="text" value={form.nombre ?? ""} onChange={e => setForm({...form, nombre: e.target.value})} className={inputClass} placeholder="Nombre y apellido" /></div>
              ...
              <div>
                ...
                {showForm && (
                  <div className="bg-[#0a1628]/80 border border-[#00FF80]/30 rounded-xl p-5 mb-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div className="md:col-span-2"><label className={labelClass}>Nombre</label><input type="text" value={form.nombre ?? ""} onChange={e => setForm({...form, nombre: e.target.value})} className={inputClass} placeholder="Ej: Factura Agroquímicos enero" /></div>
                      ...
                      return (
                        <div key={d.id} className="card-doc bg-[#0a1628]/80 border rounded-xl p-5"
                          ...
                          <div className="flex gap-3 pt-3 border-t border-[#00FF80]/10">
                            {d.archivo_url && <a href={d.archivo_url} target="_blank" rel="noreferrer" className="text-xs text-[#00FF80] font-mono">📎 Ver archivo</a>}
