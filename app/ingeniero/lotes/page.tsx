...
export default function IngenieroLotesPage() {
  ...
  // Parser FIFO: calcula cantidad total = dosis_por_ha × ha, busca match en prodsList
  const parsearInsumosDeDescripcionFifo = (desc: string, ha: number, stockList: any[], fifoList: any[]): DescuentoItem[] => {
    if (!desc || !stockList.length) return [];
    const CANT_RE = /([0-9]+[,.]?[0-9]*)\s*(litros?|lts?|lt|cc|ml|kg|grs?|gr|g|l)(?=[\s\/+\-,]|$)/gi;
    const segmentos = desc.split(/\s*\+\s*/);
    const segsParseados: Array<{palabras:string[];cantHa:number}> = [];
    for (const seg of segmentos) {
    ...
    for (const ins of stockList) {
      ...
      usados.add(bestIdx);
      const cantTotal = Math.round(segsParseados[bestIdx].cantHa * ha * 100) / 100;
      // Calcular PPP desde fifo
      const ls = fifoList.filter((l:any)=>l.producto_id===ins.id&&Number(l.cantidad_restante)>0&&Number(l.precio_usd)>0);
      ...
      items.push({
        ...
        cantidad_sugerida: cantTotal, cantidad_ajustada: cantTotal,
        precio_ppp: ppp, costo_total: Math.round(cantTotal * ppp * 100)/100, seleccionado: true,
      });
    }
    return items;
  };
  ...
  const abrirPanelDescuento = async (laborPayload: any, ha: number, desc: string) => {
   const sb = await getSB();
    ...
    // Calcular PPP y stock por producto
    ...
    // Poblar insumosStock con productos del nuevo sistema
    const stockList = prodsList.map((p: any) => ({
      id: p.id, nombre: p.nombre, unidad: p.unidad, categoria: p.categoria,
      cantidad: calcStock(p.id),
      precio_ppp: calcPpp(p.id),
      precio_unitario: 0,
    }));
    setInsumosStock(stockList as InsumoStock[]);

    // Parsear descripción para auto-calcular cantidades (dosis/ha × ha)
    const sugeridos = parsearInsumosDeDescripcionFifo(desc, ha, stockList, fifoList);

    if (sugeridos.length === 0) {
      setDescuentoItems(prodsList.map((p: any) => ({
        insumo_id: p.id, nombre: p.nombre, unidad: p.unidad,
        cantidad_sugerida: 0, cantidad_ajustada: 0,
        precio_ppp: calcPpp(p.id), costo_total: 0, seleccionado: false,
      })));
    } else {
      setDescuentoItems(sugeridos);
    }

    // Detectar productos mencionados que NO están en insumos_productos
    ...
    for (const seg of segDesc) {
      ...
      const yaEnProd = prodsList.some((p: any) => {
        const n = p.nombre.toLowerCase().replace(/[^a-z0-9]/g," ");
  ...
  const confirmarDescuento = async () => {
   ...
   const sb = await getSB();
    ...
    for (const item of itemsSeleccionados) {
      // Lotes FIFO del producto ordenados por fecha (más antiguo primero)
      ...
      // Consumir lotes FIFO disponibles
      ...
      // Si no alcanzó el stock → crear lote negativo
      if (restante > 0) {
    ...
    // Crear entradas negativas para productos no encontrados
  ...
  const guardarLabor = async () => {
    ...
    const tipoLaborNotif = form.tipo_lab ?? "Aplicación";
