...
type Visita = {
  ...
  descripcion: string; lotes: string; observaciones: string; costo: number;
};
type Cobranza = {
...
const ROL_PREFIJOS: Record<string,{label:string;color:string;icon:string}> = {
  ingeniero:{label:"Ingeniero",color:"#60A5FA",icon:"👨‍💼"},
  veterinario:{label:"Veterinario",color:"#A78BFA",icon:"🩺"},
  aplicador:{label:"Aplicador",color:"#C9A227",icon:"💧"},
  sembrador:{label:"Sembrador",color:"#4ADE80",icon:"🌱"},
  empleado:{label:"Empleado",color:"#FB923C",icon:"👷"},
};
...
export default function IngenieroPanel() {
  ...
  const calcularAlertas = (vehs: Vehiculo[], cobs: Cobranza[]) => {
    ...
    vehs.forEach(v => {
      ...
      if (v.proximo_service_km > 0 && v.km_actuales >= v.proximo_service_km - 500)
        alerts.push({ msg: v.nombre+": Service proximo", urgencia:"media" });
    });
    setAlertas(alerts);
  };
  ...
  const guardarPerfil = async () => {
    ...
    await sb.from("usuarios").update({
      nombre: form.nombre ?? ingenieroData.nombre,
      telefono: form.telefono ?? ingenieroData.telefono ?? "",
      matricula: form.matricula ?? ingenieroData.matricula ?? "",
      especialidad: form.especialidad ?? ingenieroData.especialidad ?? "",
      cuit: form.cuit ?? ingenieroData.cuit ?? "",
      direccion: form.direccion ?? ingenieroData.direccion ?? "",
      localidad: form.localidad ?? ingenieroData.localidad ?? "",
      provincia: form.provincia ?? ingenieroData.provincia ?? "",
    }).eq("id", ingenieroId);
    msg("✅ PERFIL GUARDADO");
    const sb2 = await getSB();
    const { data: updated } = await sb2.from("usuarios").select("*").eq("id", ingenieroId).single();
    if (updated) setIngenieroData(updated);
  };
  ...
  const guardarVisita = async () => {
    ...
    const sb = await getSB();
    await sb.from("ing_visitas").insert({
      ingeniero_id: ingenieroId, productor_id: form.productor_id_v,
      fecha: form.fecha_v ?? new Date().toISOString().split("T")[0],
      tipo_servicio: form.tipo_servicio ?? "Asesoramiento",
      descripcion: form.descripcion_v ?? "", lotes: form.lotes_v ?? "",
      observaciones: form.obs_v ?? "", costo: Number(form.costo_v ?? 0),
    });
    msg("✅ VISITA REGISTRADA");
    await fetchAll(ingenieroId); setShowFormVisita(false); setForm({});
  };
  ...
  const exportarExcel = async (tipo: "productores"|"lotes"|"visitas") => {
    ...
    let data: any[] = [];
    if (tipo === "productores") {
      data = productores.map(p => ({ NOMBRE:p.nombre, TELEFONO:p.telefono, EMAIL:p.email, LOCALIDAD:p.localidad, HA:p.hectareas_total, HONORARIO_TIPO:p.honorario_tipo, HONORARIO_MONTO:p.honorario_monto, TIENE_CUENTA_APP:p.tiene_cuenta?"SI":"NO" }));
    } else if (tipo === "lotes") {
      let lf = todosLotes;
      if (filterCultivo !== "todos") lf = lf.filter(l => (l.cultivo_completo||l.cultivo) === filterCultivo);
      if (filterProductor !== "todos") lf = lf.filter(l => l.productor_nombre === filterProductor);
      if (filterEstado !== "todos") lf = lf.filter(l => l.estado === filterEstado);
      data = lf.map(l => ({ PRODUCTOR:l.productor_nombre, LOTE:l.nombre, HA:l.hectareas, CULTIVO:l.cultivo_completo||l.cultivo, ESTADO:l.estado, SIEMBRA:l.fecha_siembra||"", VARIEDAD:l.variedad||"" }));
    } else {
      data = visitas.map(v => { const p = productores.find(x=>x.id===v.productor_id); return { PRODUCTOR:p?.nombre??"—", FECHA:v.fecha, SERVICIO:v.tipo_servicio, DESCRIPCION:v.descripcion, LOTES:v.lotes, COSTO:v.costo }; });
    }
  ...
  const leerExcelProductores = async (file: File) => {
    ...
    try {
      ...
      const preview = rows.slice(1).filter((r: any) => r[cn>=0?cn:0]).map((r: any) => ({
        nombre: String(r[cn>=0?cn:0]).trim(), telefono: ct>=0?String(r[ct]).trim():"",
        email: ce>=0?String(r[ce]).trim():"", localidad: cl>=0?String(r[cl]).trim():"",
      ...
      setImportPreview(preview); setImportMsg("✅ "+preview.length+" PRODUCTORES");
  ...
  const confirmarImportProductores = async () => {
    ...
    await fetchAll(ingenieroId); setImportPreview([]); setImportMsg(""); setShowImport(false);
  };
  ...
  const guardarCobranza = async () => {
    ...
    await fetchAll(ingenieroId); setShowForm(false); setForm({}); msg("✅ COBRO REGISTRADO");
  };
  const marcarCobrado = async (id: string) => { const sb = await getSB(); await sb.from("ing_cobranzas").update({estado:"cobrado"}).eq("id",id); if(ingenieroId)await fetchAll(ingenieroId); };

  // Vehiculo
  const guardarVehiculo = async () => {
    if (!ingenieroId||!form.nombre?.trim()) return;
    const sb = await getSB();
    await sb.from("ing_vehiculos").insert({ ingeniero_id:ingenieroId, nombre:form.nombre, marca:form.marca??"", modelo:form.modelo??"", año:Number(form.anio??0), patente:form.patente??"", seguro_vencimiento:form.seguro_vencimiento||null, seguro_compania:form.seguro_compania??"", vtv_vencimiento:form.vtv_vencimiento||null, km_actuales:Number(form.km_actuales??0), proximo_service_km:Number(form.proximo_service_km??0) });
    await fetchAll(ingenieroId); setShowForm(false); setForm({}); msg("✅ VEHICULO GUARDADO");
  };
  ...
  const eliminar = async (tabla: string, id: string) => { if(!confirm("Eliminar?"))return; const sb=await getSB(); await sb.from(tabla).delete().eq("id",id); if(ingenieroId)await fetchAll(ingenieroId); };
  ...
  const interpretarVoz = useCallback(async (texto: string) => {
    ...
    const resumen = productores.slice(0,5).map(p=>p.nombre+" ("+p.hectareas_total+"ha)").join("; ");
    try {
      const res = await fetch("/api/scanner",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:400,messages:[{role:"user",content:"Asistente ingeniero agronomo. Productores: "+resumen+". Usuario: \""+texto+"\". JSON sin markdown: {\"texto\":\"respuesta breve\",\"accion\":\"consulta|crear_productor\",\"datos\":{}}"}]})});
      const data = await res.json();
      const parsed = JSON.parse((data.content?.[0]?.text??"{}").replace(/```json|```/g,"").trim());
      setVozRespuesta(parsed.texto??""); hablar(parsed.texto??"");
      if (parsed.accion==="crear_productor"&&parsed.datos) { setForm({nombre:parsed.datos.nombre??"",localidad:parsed.datos.localidad??"",hectareas_total:String(parsed.datos.hectareas??"")}); setShowForm(true); }
  ...
  const askAI = async () => {
    ...
    try {
      ...
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1500,system:"Asistente agronomico experto para ingenieros en Argentina. Respondé en español tecnico. Ingeniero: "+ingenieroNombre+". Productores: "+productores.length+".",messages:[...hist,{role:"user",content:userMsg}]})});
      const data=await res.json();
  ...
  if (loading) return <div className="min-h-screen bg-[#020810] flex items-center justify-center text-[#00FF80] font-mono animate-pulse">CARGANDO...</div>;
    ...
    <div className="relative min-h-screen bg-[#020810] text-[#E5E7EB]">
      ...
      <div className="relative z-10 bg-[#020810]/95 border-b border-[#00FF80]/20 px-6 py-3 flex items-center gap-4">
        ...
        <div className="text-right">
          ...
          <div className="text-xs text-[#60A5FA] font-mono">INGENIERO AGRONOMO · COD {ingenieroData.codigo}</div>
        </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-6">
        ...
        {alertas.length > 0 && <div className="bg-[#0a1628]/80 border border-[#F87171]/30 rounded-xl p-4 mb-5"><div className="flex items-center gap-2 mb-2"><div className="w-2 h-2 rounded-full bg-[#F87171] animate-pulse"/><span className="text-[#F87171] text-xs font-mono font-bold">ALERTAS ({alertas.length})</span></div><div className="flex flex-wrap gap-2">{alertas.map((a,i)=><div key={i} className={"px-3 py-1.5 rounded-lg text-xs font-mono border "+(a.urgencia==="alta"?"border-[#F87171]/30 text-[#F87171] bg-[#F87171]/5":"border-[#C9A227]/30 text-[#C9A227] bg-[#C9A227]/5")}>{a.urgencia==="alta"?"🔴":"🟡"} {a.msg}</div>)}</div></div>}
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {SECCIONES.map(s=><button key={s.key} onClick={()=>{setSeccion(s.key);setShowForm(false);setForm({});setVehiculoSel(null);}} className={"px-5 py-2.5 rounded-xl border text-sm font-mono transition-all font-bold "+(seccion===s.key?"sec-active":"border-[#00FF80]/15 text-[#4B5563] hover:text-[#9CA3AF]")}>{s.icon} {s.label}</button>)}
        </div>
        ...
        {seccion==="perfil" && (
          <div className="card-ing p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div><label className={lCls}>NOMBRE COMPLETO</label><input type="text" defaultValue={ingenieroData.nombre??""} onChange={e=>setForm({...form,nombre:e.target.value})} className={iCls}/></div>
              <div><label className={lCls}>TELEFONO / WA</label><input type="text" defaultValue={ingenieroData.telefono??""} onChange={e=>setForm({...form,telefono:e.target.value})} className={iCls} placeholder="3400..."/></div>
              <div><label className={lCls}>MATRICULA / COLEGIO</label><input type="text" defaultValue={ingenieroData.matricula??""} onChange={e=>setForm({...form,matricula:e.target.value})} className={iCls} placeholder="MAT 1234"/></div>
              <div><label className={lCls}>ESPECIALIDAD</label><input type="text" defaultValue={ingenieroData.especialidad??""} onChange={e=>setForm({...form,especialidad:e.target.value})} className={iCls} placeholder="Cereales, oleaginosas..."/></div>
              <div><label className={lCls}>CUIT</label><input type="text" defaultValue={ingenieroData.cuit??""} onChange={e=>setForm({...form,cuit:e.target.value})} className={iCls} placeholder="20-12345678-9"/></div>
              <div><label className={lCls}>LOCALIDAD</label><input type="text" defaultValue={ingenieroData.localidad??""} onChange={e=>setForm({...form,localidad:e.target.value})} className={iCls}/></div>
              <div><label className={lCls}>PROVINCIA</label><input type="text" defaultValue={ingenieroData.provincia??"Santa Fe"} onChange={e=>setForm({...form,provincia:e.target.value})} className={iCls}/></div>
              <div className="md:col-span-2"><label className={lCls}>DIRECCION</label><input type="text" defaultValue={ingenieroData.direccion??""} onChange={e=>setForm({...form,direccion:e.target.value})} className={iCls}/></div>
            </div>
            <div className="mt-4 p-4 bg-[#020810]/40 rounded-xl border border-[#60A5FA]/15">
              <div className="grid grid-cols-3 gap-4 text-xs font-mono">
                <div className="text-center"><div className="text-[#4B5563]">CODIGO</div><div className="text-[#60A5FA] font-bold text-lg mt-1">{ingenieroData.codigo}</div></div>
                <div className="text-center"><div className="text-[#4B5563]">EMAIL</div><div className="text-[#E5E7EB] mt-1">{ingenieroData.email}</div></div>
                <div className="text-center"><div className="text-[#4B5563]">ROL</div><div className="text-[#60A5FA] font-bold mt-1">INGENIERO</div></div>
              </div>
            </div>
          ...
          <div>
            ...
            {showFormVincular && (
              <div className="card-ing p-5 mb-4">
                ...
                <p className="text-xs text-[#4B5563] font-mono mb-3">El productor debe tener cuenta en AgroGestion PRO. Ingresa su codigo (ej: 10001)</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div><label className={lCls}>HONORARIO TIPO</label><select value={form.honorario_tipo??"mensual"} onChange={e=>setForm({...form,honorario_tipo:e.target.value})} className={iCls}><option value="mensual">Mensual</option><option value="por_ha">Por HA</option><option value="por_campana">Por campaña</option><option value="por_servicio">Por servicio</option><option value="otro">Otro</option></select></div>
            ...
            {showFormVisita && (
              <div className="card-ing p-5 mb-4">
                <h3 className="text-[#C9A227] font-mono text-sm font-bold mb-4">+ REGISTRAR VISITA / SERVICIO</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div><label className={lCls}>TIPO DE SERVICIO</label><select value={form.tipo_servicio??"Asesoramiento"} onChange={e=>setForm({...form,tipo_servicio:e.target.value})} className={iCls}>{TIPOS_SERVICIO.map(t=><option key={t} value={t}>{t.toUpperCase()}</option>)}</select></div>
                  ...
                  <div className="md:col-span-2"><label className={lCls}>DESCRIPCION</label><input type="text" value={form.descripcion_v??""} onChange={e=>setForm({...form,descripcion_v:e.target.value})} className={iCls} placeholder="Detalle del servicio..."/></div>
                  <div><label className={lCls}>LOTES</label><input type="text" value={form.lotes_v??""} onChange={e=>setForm({...form,lotes_v:e.target.value})} className={iCls} placeholder="El Norte, La Cañada..."/></div>
            ...
            {showImport && (
              <div className="card-ing p-5 mb-4">
                <div className="flex items-center justify-between mb-3"><h3 className="text-[#C9A227] font-mono text-sm font-bold">📥 IMPORTAR PRODUCTORES</h3><button onClick={()=>{setShowImport(false);setImportPreview([]);setImportMsg("");}} className="text-[#4B5563] text-sm">✕</button></div>
                ...
                {importPreview.length===0?<button onClick={()=>importRef.current?.click()} className="flex items-center gap-2 px-4 py-3 border border-dashed border-[#C9A227]/40 rounded-xl text-[#C9A227] font-mono text-sm w-full justify-center">📁 SELECCIONAR ARCHIVO</button>:(
                  <div><div className="max-h-32 overflow-y-auto mb-3 rounded-lg border border-[#C9A227]/15"><table className="w-full text-xs"><thead><tr className="border-b border-[#C9A227]/10">{["NOMBRE","TEL","EMAIL","LOCALIDAD","HA","ESTADO"].map(h=><th key={h} className="text-left px-3 py-2 text-[#4B5563] font-mono">{h}</th>)}</tr></thead><tbody>{importPreview.map((r,i)=><tr key={i} className="border-b border-[#C9A227]/5"><td className="px-3 py-2 text-[#E5E7EB] font-mono font-bold">{r.nombre}</td><td className="px-3 py-2 text-[#9CA3AF] font-mono">{r.telefono||"—"}</td><td className="px-3 py-2 text-[#9CA3AF] font-mono">{r.email||"—"}</td><td className="px-3 py-2 text-[#9CA3AF] font-mono">{r.localidad||"—"}</td><td className="px-3 py-2 text-[#C9A227] font-mono">{r.hectareas_total||"—"}</td><td className="px-3 py-2"><span className={r.existe?"text-[#60A5FA] font-mono text-xs":"text-[#4ADE80] font-mono text-xs"}>{r.existe?"Ya existe":"Nuevo"}</span></td></tr>)}</tbody></table></div>
                  <div className="flex gap-3"><button onClick={confirmarImportProductores} className="bg-[#C9A227]/10 border border-[#C9A227]/30 text-[#C9A227] font-bold px-4 py-2 rounded-lg text-xs font-mono">▶ IMPORTAR {importPreview.filter(p=>!p.existe).length} NUEVOS</button><button onClick={()=>setCuadernoPreview?setCuadernoPreview([]):setImportPreview([])} className="border border-[#1C2128] text-[#4B5563] px-4 py-2 rounded-lg text-xs font-mono">CANCELAR</button></div></div>
                )}
                {importMsg&&<p className={"mt-2 text-xs font-mono "+(importMsg.startsWith("✅")?"text-[#4ADE80]":"text-[#F87171]")}>{importMsg}</p>}
              </div>
            )}

            {/* Form nuevo productor */}
            {showForm && (
              <div className="card-ing p-5 mb-4">
                ...
                {form.email && <div className="mt-3 p-3 bg-[#60A5FA]/5 border border-[#60A5FA]/20 rounded-xl text-xs font-mono text-[#60A5FA]">ℹ️ Si el email tiene cuenta en la app, los lotes se vincularan automaticamente.</div>}
                <div className="flex gap-3 mt-4">
            ...
            {/* Exportar lotes filtrados */}
            {todosLotes.length > 0 && (
              <div className="card-ing p-4 mb-4">
                <span className="text-[#C9A227] font-mono text-sm font-bold">📊 EXPORTAR LOTES</span>
                <div className="flex flex-wrap gap-3 items-end mt-3">
                  ...
                  <button onClick={()=>exportarExcel("lotes")} className="px-5 py-2.5 rounded-xl bg-[#4ADE80]/10 border border-[#4ADE80]/30 text-[#4ADE80] font-mono text-sm font-bold hover:bg-[#4ADE80]/20">📤 EXPORTAR ({todosLotes.filter(l=>filterCultivo==="todos"||(l.cultivo_completo||l.cultivo)===filterCultivo).filter(l=>filterProductor==="todos"||l.productor_nombre===filterProductor).filter(l=>filterEstado==="todos"||l.estado===filterEstado).length} lotes)</button>
                  <div className="flex gap-2 flex-wrap">{cultivosUnicos.map(c=><button key={c} onClick={()=>{setFilterCultivo(c);setTimeout(()=>exportarExcel("lotes"),100);}} className="px-3 py-1.5 rounded-lg text-xs font-mono border border-[#C9A227]/30 text-[#C9A227] bg-[#C9A227]/5 font-bold">📊 {c}</button>)}</div>
                </div>
              </div>
            )}
            ...
            {productores.length===0?(
              <div className="text-center py-20 card-ing"><div className="text-5xl mb-4 opacity-20">👨‍🌾</div><p className="text-[#4B5563] font-mono">SIN PRODUCTORES — AGREGA O VINCULA UNO</p></div>
            ):(
              <div>
                    ...
                    <div key={p.id} className="card-ing overflow-hidden">
                      <div className="p-5">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs font-mono mb-3">
                          <div className="bg-[#020810]/40 rounded-lg p-2 text-center"><div className="text-[#4B5563]">HA</div><div className="font-bold text-[#C9A227] mt-0.5">{p.hectareas_total.toLocaleString("es-AR")}</div></div>
                          <div className="bg-[#020810]/40 rounded-lg p-2 text-center"><div className="text-[#4B5563]">HONORARIO</div><div className="font-bold text-[#00FF80] mt-0.5">${p.honorario_monto.toLocaleString("es-AR")}</div></div>
                        </div>
                ...
                <div className="card-ing overflow-hidden">
                  <div className="px-5 py-3 border-b border-[#C9A227]/15 flex items-center justify-between">
                    <span className="text-[#C9A227] font-mono text-sm font-bold">📋 HISTORIAL DE VISITAS Y SERVICIOS</span>
                    ...
                    <table className="w-full"><thead><tr className="border-b border-[#C9A227]/10">{["FECHA","PRODUCTOR","SERVICIO","DESCRIPCION","LOTES","COSTO",""].map(h=><th key={h} className="text-left px-4 py-2.5 text-xs text-[#4B5563] font-mono">{h}</th>)}</tr></thead>
                      <tbody>{visitas.slice(0,20).map(v=>{const p=productores.find(x=>x.id===v.productor_id);return(
                        <tr key={v.id} className="border-b border-[#C9A227]/5 hover:bg-[#C9A227]/5">
                          ...
                          <td className="px-4 py-3"><button onClick={()=>eliminar("ing_visitas",v.id)} className="text-[#4B5563] hover:text-red-400 text-xs">✕</button></td>
                        </tr>
                      );})}
                      </tbody>
                    </table>
        ...
        {seccion==="vehiculo" && (
          <div>
            <div className="flex items-center justify-between mb-5 flex-wrap gap-4">
              ...
              {!vehiculoSel?<button onClick={()=>{setShowForm(true);setForm({});}} className="px-4 py-2 rounded-xl bg-[#00FF80]/10 border border-[#00FF80]/30 text-[#00FF80] font-mono text-sm font-bold hover:bg-[#00FF80]/20">+ AGREGAR</button>:(
            ...
            {!vehiculoSel?(
              vehiculos.length===0?<div className="text-center py-20 card-ing"><div className="text-5xl mb-4 opacity-20">🚗</div><p className="text-[#4B5563] font-mono">SIN VEHICULOS</p></div>:(
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {vehiculos.map(v=>{const segVenc=v.seguro_vencimiento&&new Date(v.seguro_vencimiento)<new Date();const vtvVenc=v.vtv_vencimiento&&new Date(v.vtv_vencimiento)<new Date();return(
        ...
        {seccion==="ia_campo" && (
          <div>
            <div className="mb-5"><h2 className="text-lg font-bold font-mono text-[#E5E7EB]">🤖 IA CAMPO — ASISTENTE AGRONOMICO</h2><p className="text-xs text-[#4B5563] font-mono mt-1">Consulta sobre dosis, plagas, enfermedades, cultivos y mercados</p></div>
            {aiChat.length===0&&<div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-5">{["Dosis glifosato soja post-emergencia","Identificar roya asiatica soja","Fungicida manchas foliares maiz","Recomendaciones siembra trigo pampeana","Cuando aplicar insecticida soja MIP","Precio soja mercado actual"].map(q=><button key={q} onClick={()=>setAiInput(q)} className="text-left text-xs text-[#4B6B5B] hover:text-[#00FF80] border border-[#00FF80]/10 hover:border-[#00FF80]/30 px-4 py-3 rounded-xl font-mono transition-all bg-[#0a1628]/60">💬 {q}</button>)}</div>}
            <div className="card-ing overflow-hidden mb-4">
              <div className="px-5 py-3 border-b border-[#00FF80]/10 flex items-center justify-between"><div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#00FF80] animate-pulse"/><span className="text-[#00FF80] text-xs font-mono">◆ IA AGRONOMICA ACTIVA</span></div>{aiChat.length>0&&<button onClick={()=>setAiChat([])} className="text-xs text-[#4B5563] font-mono">Limpiar</button>}</div>
              <div className="p-4 space-y-4 max-h-96 overflow-y-auto">
                ...
                {aiChat.map((m,i)=><div key={i} className={"flex "+(m.rol==="user"?"justify-end":"justify-start")}><div className={"max-w-[80%] px-4 py-3 rounded-xl text-sm font-mono "+(m.rol==="user"?"bg-[#00FF80]/10 border border-[#00FF80]/20 text-[#E5E7EB]":"bg-[#0F1115] border border-[#1C2128] text-[#9CA3AF]")}>{m.rol==="assistant"&&<div className="text-[#00FF80] text-xs mb-2">◆ IA AGRONOMICA</div>}<p className="leading-relaxed whitespace-pre-wrap">{m.texto}</p></div></div>)}
            ...
            <div className="flex gap-3">
              ...
              <button onClick={askAI} disabled={aiLoading||!aiInput.trim()} className="px-6 py-3 rounded-xl bg-[#00FF80]/10 border border-[#00FF80]/30 text-[#00FF80] font-mono text-sm disabled:opacity-40 flex-shrink-0 font-bold">▶ ENVIAR</button>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Panel voz flotante */}
      {vozPanel&&<div className="fixed bottom-44 right-6 z-50 w-80 bg-[#0a1628]/97 border border-[#00FF80]/30 rounded-2xl shadow-2xl overflow-hidden backdrop-blur-sm"><div className="flex items-center justify-between px-4 py-3 border-b border-[#00FF80]/20"><div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{background:VOZ_COLOR[vozEstado]}}/><span className="text-[#00FF80] text-xs font-mono font-bold">🎤 ASISTENTE</span></div><button onClick={()=>{setVozPanel(false);window.speechSynthesis?.cancel();recRef.current?.stop();setVozEstado("idle");}} className="text-[#4B5563] text-sm">✕</button></div><div className="px-4 pt-3 pb-2 min-h-20">{vozEstado==="escuchando"&&<div className="flex items-center gap-3 py-2"><div className="flex gap-1 items-end h-8">{[1,2,3,4,5].map(i=><div key={i} className="w-1.5 rounded-full bg-[#F87171]" style={{height:(10+i*5)+"px"}}/>)}</div><span className="text-[#F87171] text-sm font-mono">ESCUCHANDO...</span></div>}{vozRespuesta&&<div className="bg-[#00FF80]/8 border border-[#00FF80]/20 rounded-lg px-3 py-2 mb-2"><p className="text-[#E5E7EB] text-sm font-mono">{vozRespuesta}</p></div>}{!vozRespuesta&&!vozTranscripcion&&vozEstado==="idle"&&<div className="space-y-1 py-1">{["CUANTOS PRODUCTORES TENGO","NUEVO PRODUCTOR JUAN PEREZ 200 HAS RAFAELA"].map(q=><button key={q} onClick={()=>{setVozTranscripcion(q);interpretarVoz(q);}} className="w-full text-left text-xs text-[#4B6B5B] hover:text-[#00FF80] border border-[#00FF80]/10 px-3 py-2 rounded-lg font-mono">💬 {q}</button>)}</div>}</div><div className="px-3 pb-3 flex gap-2 border-t border-[#00FF80]/10 pt-3"><input value={vozInput} onChange={e=>setVozInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&vozInput.trim()){setVozTranscripcion(vozInput);interpretarVoz(vozInput);setVozInput("");}}} placeholder="Escribi o habla..." className="flex-1 bg-[#020810]/80 border border-[#00FF80]/20 rounded-lg px-3 py-2 text-[#E5E7EB] text-xs font-mono focus:outline-none focus:border-[#00FF80]"/><button onClick={()=>{if(vozEstado==="escuchando"){recRef.current?.stop();setVozEstado("idle");}else escucharVoz();}} className="px-3 py-2 rounded-lg text-sm" style={{background:VOZ_COLOR[vozEstado]+"20",border:"1px solid "+VOZ_COLOR[vozEstado],color:VOZ_COLOR[vozEstado]}}>{VOZ_ICON[vozEstado]}</button>{vozInput&&<button onClick={()=>{setVozTranscripcion(vozInput);interpretarVoz(vozInput);setVozInput("");}} className="px-3 py-2 rounded-lg bg-[#00FF80]/10 border border-[#00FF80]/30 text-[#00FF80] text-xs font-mono">▶</button>}</div></div>}
      ...
      <p className="relative z-10 text-center text-[#0a2a1a] text-xs pb-4 font-mono mt-6">AGROGESTION PRO · PANEL INGENIERO AGRONOMO</p>
